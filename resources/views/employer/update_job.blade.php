...
<form action="{{route('work.update',$work)}}" method="post">
    ...
    <div class="rounded-md border border-slate-300 p-4  mt-2 bg-white dark:bg-gray-800 shadow">
        ...
        @enderror
            ...
            <x-form-input type="number" name="salary" value="{{$work->salary}}"
                placeholder="Enter salary"></x-form-input>
                @error('salary')
            <span class="text-sm text-red-600">{{$message}}</span>
        @enderror
        ...
        @enderror
           ...
           <input type="radio" name="category" value="{{ $category }}" id="{{ $category }}" @checked($work->category=== $category)>{{$category}}
           @endforeach
            @error('category')
            <span class="text-sm text-red-600">{{$message}}</span>
        @enderror
           ...
           <input type="radio" name="experience" value="{{ $experience }}" id="{{ $experience }}" @checked($work->experience=== $experience)>{{$experience}}
           @endforeach
            @error('experience')
            <span class="text-sm text-red-600">{{$message}}</span>
        @enderror
